...
ユーザーが「旅行地図を作成して」「旅程地図を作成して」「Travel2026の地図を更新して」または同等の依頼をした場合は、Remote経由の短い指示を含め、リポジトリスキル `$travel-map-update` を使用する。このスキルを反復的なGitHub公開ワークフローの正本とし、Remote経由でも取得、検証、公開確認を省略しない。`index.html` は引き続き保護対象であり、変更しない。
